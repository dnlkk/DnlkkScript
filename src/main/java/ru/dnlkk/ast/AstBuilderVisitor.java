...
import ru.dnlkk.DnlkkScriptLanguageServer;
import ru.dnlkk.gen.DnlkkRulesBaseVisitor;
...
public class AstBuilderVisitor extends DnlkkRulesBaseVisitor<AstNode> {
    private final Map<String, Set<String>> objectFields = new HashMap<>();
    private final Set<String> allIdents = new HashSet<>();
    private final DnlkkScriptLanguageServer languageServer;

    public AstBuilderVisitor(DnlkkScriptLanguageServer languageServer) {
    ...
    public AstNode visitProgram(DnlkkRulesParser.ProgramContext ctx) {
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "Stmts amount: " + ctx.stmt_list().stmt().size()
                ));
    ...
    public AstNode visitDefinition(DnlkkRulesParser.DefinitionContext ctx) {
        ...
        allIdents.add(ident);
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ident: " + ident
                ));

        AstNode identNode = new TerminalAstNode(ident);
        AstNode value =
                ctx.expr() != null ?
                        visitExpr(ctx.expr()) :
                        visitFun(ctx.fun());

        addFieldsToObject(value, ident); // Рекурсивное добавление полей
        addIdentsToList(identNode); // Рекурсивное добавление идентификаторов
        addIdentsToList(value); // Рекурсивное добавление идентификаторов

        return new BinOpNode("var", identNode, value);
    }

    private void addIdentsToList(AstNode node) {
        if (node instanceof TerminalAstNode) {
            allIdents.add(node.getName());
        }
        for (AstNode child : node.getChildren()) {
            addIdentsToList(child); // Рекурсивный вызов для всех детей
        }
    }


    private void addFieldsToObject(AstNode node, String ident) {
        if (node instanceof ObjectLiteralNode) {
            Set<String> fields = new HashSet<>();
            for (AstNode field : node.getChildren()) {
                String fieldName = field.getChild(0).getName();
                fields.add(fieldName);
                addFieldsToObject(field.getChild(1), fieldName); // Рекурсивный вызов для вложенных объектов
            }
            objectFields.put(ident, fields);
        }
    }
    ...
    public AstNode visitField(DnlkkRulesParser.FieldContext ctx) {
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ctx getText: " + ctx.getText()
                ));
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ctx getRuleIndex: " + ctx.getRuleIndex()
                ));
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ctx getChild getText: " + ctx.getChild(0).getText()
                ));
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ctx getChild getPayload: " + ctx.getChild(0).getPayload()
                ));
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "ctx.IDENT(): " + ctx.IDENT()
                ));
        String fieldName = ctx.IDENT().getText();
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "fieldName: " + fieldName
                ));
        AstNode fieldNameNode = new TerminalAstNode(fieldName);
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "fieldNameNode: " + fieldNameNode
                ));

        AstNode value = ctx.expr() != null ? visitExpr(ctx.expr()) : visitFun(ctx.fun());
        languageServer.getLanguageClient().logMessage(
                new MessageParams(MessageType.Info, "value: " + value
                ));

        return new FieldNode(fieldNameNode, value);
    }

    @Override
    public AstNode visitAssign(DnlkkRulesParser.AssignContext ctx) {
        String ident = ctx.call().getText();
        allIdents.add(ident);
        ...
        addFieldsToObject(value, ident); // Рекурсивное добавление полей
        addIdentsToList(variable); // Рекурсивное добавление идентификаторов
        addIdentsToList(value); // Рекурсивное добавление идентификаторов

        return new AssignNode(variable, value);
    }

    public Map<String, Set<String>> getObjectFields() {
        return objectFields;
    }

    public Set<String> getAllIdents() {
        return allIdents;
    }
